function enDeCode(){
    let userString = document.getElementById("uMessage").value;
    let encode = document.getElementById("encode");
    let userKey = document.getElementById("userKey").value;
    let cleanString = (userString.trim()).toLowerCase();
    let outputMessage = [];
    let flag = true;
  // code function, gets each element ready before running encoding/decoding
    if (encode.checked) {
      flag = true;
    } else{
    flag = false;
  ...
  // if statement dictates the initial (start point) of the phrase, before adding a set amount of characters through .length
  ...
  // index of characters used in the cipher
  ...
  // contingency to stop edge case errors (z not being recognised, there are additional rules below this note regarding)
